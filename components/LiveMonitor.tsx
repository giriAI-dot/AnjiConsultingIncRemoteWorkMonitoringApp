...
// Robust UUID generator for non-secure contexts
const generateUUID = () => {
...
export const LiveMonitor: React.FC<LiveMonitorProps> = ({ username, onSessionComplete }) => {
  ...
  const stateRef = useRef({ status, elapsedTime, logs, activeSessionId: '' });
  ...
  useEffect(() => {
    stateRef.current = { status, elapsedTime, logs, activeSessionId };
  }, [status, elapsedTime, logs, activeSessionId]);

  useEffect(() => {
    isCameraEnabledRef.current = isCameraEnabled;
  }, [isCameraEnabled]);

  // Save chunks and metadata on tab close
  useEffect(() => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (status === MonitoringState.RECORDING || status === MonitoringState.PAUSED) {
        saveSessionToStorage(elapsedTime, logs, chunksRef.current, activeSessionId);
      }
    }, 10000);
  ...
  const saveSessionToStorage = (time: number, currentLogs: SessionLog[], chunks: Blob[], sessionId: string) => {
      const meta = {
          elapsedTime: time,
      ...
      if (currentLogs.length > 0) {
          saveLogsToDB(sessionId, currentLogs);
      }
  };
  ...
  const handleResumeSession = async () => {
      if (recoveryMeta) {
          ...
          if (recoveryMeta.activeSessionId) {
              setActiveSessionId(recoveryMeta.activeSessionId);
          }
  ...
  const handleDiscardSession = async () => {
      ...
      setShowRecovery(false);
      setRecoveryMeta(null);
      setActiveSessionId(generateUUID()); // Ensure fresh ID for next
  };
  ...
  const startMonitoring = async () => {
    try {
              ...
              const segInterval = window.setInterval(async () => {
                  if (segmentationRef.current && dummyVideo.readyState >= 2 && isSegmentationReadyRef.current) {
                       if (bgConfigRef.current.mode !== 'none') {
                           try { await segmentation.send({image: dummyVideo}); } catch(e) {}
                       }
                  }
              }, 33);
      ...
      try {
          ...
          fallbackCanvas.width = 1280; fallbackCanvas.height = 720;
          const ctx = fallbackCanvas.getContext('2d');
          if (ctx) {
              ctx.fillStyle = '#1f2937'; ctx.fillRect(0, 0, 1280, 720);
              ctx.fillStyle = '#9ca3af'; ctx.font = 'bold 30px sans-serif';
      ...
      if (compCanvas) {
          ...
          const drawCompositor = () => {
              ...
              if (isCameraEnabled && camVideo && camVideo.readyState >= 2) {
                  const pipW = 320; const pipH = 180;
                  const pipX = 1280 - pipW - 20; const pipY = 720 - pipH - 20;
                  ctx.save();
      ...
      const audioTracks = [...userStream.getAudioTracks(), ...screenStream.getAudioTracks()];
      audioTracks.forEach(track => recordingStream.addTrack(track));
      streamRef.current = recordingStream;
      const recorder = new MediaRecorder(recordingStream, { mimeType: 'video/webm;codecs=vp8,opus' });
      ...
      if (screenVideoTrack && screenVideoTrack.readyState === 'live' && !screenVideoTrack.label.includes('canvas')) {
          screenVideoTrack.onended = () => { stopMonitoring(); };
      }
    } catch (err: any) {
      console.error("Error starting capture:", err);
      if (err.name === 'NotAllowedError') alert("Permission denied.");
  ...
  const resumeMonitoring = () => {
    if (streamRef.current && streamRef.current.active) {
        ...
        lastActivityRef.current = Date.now(); setIsIdle(false); startAnalysisLoop();
        setStatus(MonitoringState.RECORDING);
  ...
  const stopMonitoring = useCallback(async () => {
    ...
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      mediaRecorderRef.current.onstop = async () => {
          ...
          if (logs.length > 0) await saveLogsToDB(activeSessionId, logs);
          setElapsedTime(0); setLogs([]); setCurrentAnalysis(null);
          chunksRef.current = [];
          setActiveSessionId(generateUUID()); // Fresh ID for next session
      };
      mediaRecorderRef.current.stop();
    } else {
        setElapsedTime(0); setLogs([]); setCurrentAnalysis(null);
        chunksRef.current = []; localStorage.removeItem('sw_session_meta');
        setActiveSessionId(generateUUID());
    }
    setStatus(MonitoringState.IDLE); setIsCameraEnabled(true); setIsIdle(false);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:h-[calc(100vh-10rem)] relative">
      ...
      <div className="lg:col-span-2 flex flex-col gap-4">
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-1 relative overflow-hidden group shadow-2xl">
             ...
             {status === MonitoringState.IDLE ? (
                 ...
                 <>
                    ...
                    <div className="absolute bottom-2 right-2 sm:bottom-4 sm:right-4 w-24 sm:w-40 md:w-48 aspect-video bg-gray-950 rounded-lg overflow-hidden border border-gray-700 shadow-xl z-20 transition-opacity duration-300 group/pip">
                        <video ref={cameraVideoRef} autoPlay playsInline muted className={`w-full h-full object-cover transition-opacity duration-300 ${isCameraEnabled ? 'opacity-100' : 'opacity-0'}`} />
          ...
          <div className={`w-full p-4 flex flex-wrap gap-3 justify-center items-center bg-gray-900/95 border-t border-gray-800 z-30 lg:absolute lg:bottom-6 lg:left-1/2 lg:-translate-x-1/2 lg:w-auto lg:bg-transparent lg:border-none lg:p-0 lg:transition-opacity lg:duration-300 ${status === MonitoringState.IDLE ? 'lg:opacity-0 lg:group-hover:opacity-100' : 'lg:opacity-100'}`}>
             {status === MonitoringState.IDLE ? (
                ...
                <>
                    ...
                    <button onClick={toggleCamera} className="bg-gray-700 hover:bg-gray-600 text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-full font-medium text-xs sm:text-sm whitespace-nowrap">{isCameraEnabled ? 'Hide Cam' : 'Show Cam'}</button>
                    <div className="relative">
                        <button onClick={() => setShowBgSettings(!showBgSettings)} className={`bg-gray-800 hover:bg-gray-700 text-white px-3 py-2 sm:py-2.5 rounded-full font-medium text-sm border border-gray-600 ${showBgSettings ? 'ring-2 ring-indigo-500' : ''}`} title="Camera Settings">
                        ...
                        {showBgSettings && (
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-72 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl p-4 z-50 animate-fade-in-up">
                                ...
                                <div className="flex bg-gray-800 rounded-lg p-1 mb-4">
                                    <button onClick={() => setBgConfig(p => ({...p, mode: 'blur'}))} className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${bgConfig.mode === 'blur' ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>Blur</button>
                                    <button onClick={() => setBgConfig(p => ({...p, mode: 'image'}))} className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${bgConfig.mode === 'image' ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>Image</button>
                                    <button onClick={() => setBgConfig(p => ({...p, mode: 'none'}))} className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${bgConfig.mode === 'none' ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>None</button>
                                </div>
                                {bgConfig.mode === 'blur' && (
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-xs text-gray-400"><span>Intensity</span><span>{Math.round((bgConfig.blurRadius / 20) * 100)}%</span></div>
                                        <input type="range" min="2" max="20" value={bgConfig.blurRadius} onChange={(e) => setBgConfig(p => ({...p, blurRadius: parseInt(e.target.value)}))} className="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                                    </div>
                                )}
                                {bgConfig.mode === 'image' && (
                                    <div className="grid grid-cols-2 gap-2">
                                        <button onClick={() => setBgConfig(p => ({...p, sourceType: 'default'}))} className={`relative aspect-video rounded-lg overflow-hidden border-2 transition-all ${bgConfig.sourceType === 'default' ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-gray-700 hover:border-gray-500'}`}>
                                            <div className="absolute inset-0 bg-gray-800 flex flex-col items-center justify-center"><span className="text-[10px] text-orange-500 font-bold">ANJI</span><span className="text-[8px] text-gray-500">DEFAULT</span></div>
                                        </button>
                                        <button onClick={() => fileInputRef.current?.click()} className={`relative aspect-video rounded-lg overflow-hidden border-2 transition-all group ${bgConfig.sourceType === 'custom' ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-gray-700 hover:border-gray-500'}`}>
                                            {bgConfig.customImage ? <img src={bgConfig.customImage.src} className="w-full h-full object-cover" alt="Custom" /> : <div className="absolute inset-0 bg-gray-800 flex flex-col items-center justify-center text-gray-400"><svg className="w-5 h-5 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg><span className="text-[8px]">UPLOAD</span></div>}
                                            <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><span className="text-[10px] text-white">Change</span></div>
                                        </button>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 shadow-sm">
            <h3 className="text-gray-400 text-sm font-semibold uppercase tracking-wider mb-4 flex items-center gap-2">Real-time AI Context</h3>
            {status === MonitoringState.IDLE ? <p className="text-gray-600 italic">Start session to enable Gemini analysis...</p> : currentAnalysis ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-gray-800/50 p-4 rounded-lg"><p className="text-xs text-gray-500 mb-1">Activity</p><p className="text-white font-medium">{currentAnalysis.category}</p></div>
                    <div className="bg-gray-800/50 p-4 rounded-lg"><p className="text-xs text-gray-500 mb-1">Risk</p><p className={currentAnalysis.riskLevel === 'high' ? 'text-red-400 font-bold' : 'text-green-400'}>{currentAnalysis.riskLevel.toUpperCase()}</p></div>
                    <div className="bg-gray-800/50 p-4 rounded-lg"><p className="text-xs text-gray-500 mb-1">Summary</p><p className="text-gray-300 text-sm">{currentAnalysis.summary}</p></div>
                </div>
            ) : <div className="animate-pulse space-y-4"><div className="h-4 bg-gray-800 rounded w-3/4"></div></div>}
        </div>
      </div>
      <div className="bg-gray-900 border border-gray-800 rounded-2xl flex flex-col overflow-hidden shadow-xl lg:h-auto h-96">
        <div className="p-4 border-b border-gray-800 bg-gray-900/95 backdrop-blur z-10"><h3 className="text-white font-semibold">Compliance Log</h3></div>
